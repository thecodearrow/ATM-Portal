...
export const DepositMoney = () => {
    ...
    const depositMoney =()=>{
      const requestOptions = {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ "debitCardNumber": selectedCardNumber,"pin":pin,"amount":depositAmount})
    };
    fetch('http://localhost:8080/deposit-money', requestOptions)
        .then(response => response.json())
        .then(data => {
            if(data && data.status===true){
                //deposit success
    ...
    return (
      ...
      <FormControl >
        ...
        <TextField style={{margin:10}} id="amount" label="Amount" variant="outlined" onChange={e => setDepositAmount(e.target.value)} required/>
        
        <Button style={{margin:10}} variant="contained" onClick={depositMoney}>Deposit</Button>
      </FormControl>
      <Snackbar
        open={successOpen}
